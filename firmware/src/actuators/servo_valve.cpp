...
static const int TIME_TO_OPEN_MS  = 750;
static const int TIME_TO_CLOSE_MS = 500;
...
static void haltAndDetachServo() {
  valveServo.write(94); // 90 is the theoretical stop point for 360 servos
  delay(400);            // Brief pause to let the motor brake
...
void openValve() {
  ...
  // 1. Pre-load the speed and direction FIRST
  valveServo.write(108); 
  
  // 2. THEN re-attach the signal wire. 
  // This prevents the default 90 pulse from firing.
